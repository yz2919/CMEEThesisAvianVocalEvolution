...
pPCA.passer<- phyl.pca(tree=combined.data$phy,Y=combined.data$data,method="BM",mode="corr")

pPCAScores_passer<-pPCA.passer$S

write.csv(pPCAScores_passer,"../data/pPCAScores_passer.csv")

pPCALoadings_passer<-pPCA.passer$L

write.csv(pPCALoadings_passer,"../data/pPCALoadings_passer.csv")
